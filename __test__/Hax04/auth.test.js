...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      .post('/Hax04/auth/register')
      .send({
        firstName: 'Stuart Bogisich',
        lastName: 'Tiffany Smith',
        name: 'Erick Heller III',
        username: 'Miss Gregg Grant',
        password: 'v5PWC_LeN7oQbvE',
        email: 'Mina85@hotmail.com',
        addedby: '61360642665a2120b8fde4b6',
        role: 1
      });
...
describe('POST /login -> if username and password is correct', () => {
  test('should return user with authentication token', async () => {
    let user = await request(app)
      .post('/Hax04/auth/login')
      .send({
        password: 'v5PWC_LeN7oQbvE',
        username: 'Mina85@hotmail.com' 
      });
...
describe('POST /login -> if username is incorrect', () => {
  test('should return unauthorized status and user not exists', async () => {
    let user = await request(app)
      ...
      .send({
        password: 'v5PWC_LeN7oQbvE',
        username: 'wrongMina85@hotmail.com' 
      });
...
describe('POST /forgot-password -> if email has not passed from request body', () => {
  test('should return bad request status and insufficient parameters', async () => {
    let user = await request(app)
      .post('/Hax04/auth/forgot-password')
      .send({ email: '' });
...
describe('POST /validate-otp -> otp is sent in request body and OTP is correct', () => {
  test('should return success', async () => {
    ...
    let foundUser = await request(app)
      .get(`/Hax04/api/v1/user/${login.body.data.id}`)
      .set({
